...
This research project, developed at the **I2CS Research Lab, Indian Institute of Information Technology, Kottayam**, has been officially selected for presentation at the prestigious **Design and Verification Conference (DVCon) India 2025**. This repository contains the complete software model, quantization analysis, and verification framework that serves as the high-level counterpart to our custom-designed hardware accelerator.

---

## 1. Introduction: The Problem

Transformer models have become the state-of-the-art in many machine learning domains, including Network Intrusion Detection Systems (NIDS). However, their effectiveness comes at a high computational cost. The core of a Transformer is the **Multi-Head Attention** mechanism, which relies heavily on large matrix multiplication operations (for Query, Key, and Value matrices) and subsequent feed-forward networks.

This computational demand makes it extremely challenging to deploy these powerful models on **resource-constrained edge devices**, where power, memory, and processing capabilities are limited. Our project directly addresses this challenge by offloading these expensive computations to a dedicated, high-performance hardware accelerator implemented on an FPGA.
...
The core of our project is a custom **FPGA-based accelerator IP** designed to accelerate the matrix multiplication bottlenecks in Transformer models. Key features of our hardware design include:

* **Tiled Matrix Multiplication:** The accelerator uses a tiled computation approach, breaking large matrices into smaller 16x16 tiles. This strategy maximizes data reuse from fast on-chip memory (BRAMs) and minimizes slow, power-hungry access to external DDR3 memory.
* **Massive Parallelism:** The inner computation loops are fully unrolled using High-Level Synthesis (HLS) pragmas, creating **256 parallel Multiply-Accumulate (MCA) engines** that utilize the FPGA's DSP slices for high-throughput computation.
* **Pipelined Architecture:** The entire computation pipeline is optimized with an Initiation Interval (II) of 1, meaning it can accept new data every clock cycle, maximizing hardware utilization.
* **Optimized Memory Access:** The design features three dedicated full AXI channels for concurrently reading input matrices and writing results, eliminating memory arbitration bottlenecks and enabling efficient AXI burst transfers.
...
2.  **Quantization Analysis:** The process of converting the model from high-precision 32-bit floating-point (FP32) numbers to hardware-friendly 8-bit integers (INT8). This is crucial for reducing the model's memory footprint and enabling efficient integer-based computation on the FPGA. This repository contains the analysis for both **Post-Training Quantization (PTQ)** and **Quantization-Aware Training (QAT)**.
...
* **Hardware Simulation:** RTL cosimulation in Vivado XSIM verified the functional correctness of the accelerator. The results showed significant performance gains, with a computation time of approximately **47ms for a 128x512x512 matrix multiplication**, achieving **1.42 GFLOPs**. The tiled writeback approach also showed improved AXI bus utilization by creating periodic gaps for other system components to access memory.
...
[1] R. Li and S. Chen, "Design and Implementation of an FPGA-Based Hardware Accelerator for Transformer," arXiv preprint arXiv:2503.16731, 2025.

[2] X. Yang and T. Su, "EFA-Trans: An Efficient and Flexible Acceleration Architecture for Transformers," Electronics, vol. 11, no. 21, p. 3550, 2022.

[3] Xilinx, "Matrix Multiplication | High Level Systhesis Design Flow," GitHub. [Online]. Available: https://github.com/Xilinx/xup_high_level_synthesis_design_flow/blob/main/source/matmult/notebook/matmul_part2.ipynb.

[4] Xilinx, "Vitis HLS User Guide (UG1399)," 2022. [Online]. Available: https://www.xilinx.com/support/documents/sw_manuals/xilinx2022_2/ug1399-vitis-hls.pdf.
# FlowTransformer
